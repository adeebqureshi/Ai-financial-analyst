"""Regression tests for tenant-scoped sparse (BM25) retrieval.

Before the fix, the BM25 index was built from *all* tenants' chunks and its
results were truncated to the fused top-k BEFORE the engine's owner filter,
so chunks owned by other users could displace the caller's own chunks from
the sparse candidate list (and leak chunk IDs into the fused ranking).
"""
