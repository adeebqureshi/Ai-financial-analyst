"""
Non-blocking / concurrency regression tests for the async execution paths.

These verify that blocking, synchronous work (tool I/O, database persistence)
is offloaded to worker threads via ``asyncio.to_thread`` and never runs
directly on the event loop, while the API behaviour stays unchanged.
"""
...
class _BlockingTools:
    """
    Tool-registry stand-in whose ``execute`` blocks its worker thread the way
    real tools do (yfinance / requests HTTP, SQLAlchemy access, sandbox run).
    """
...
async def test_stream_run_offloads_blocking_tool_execution():
    """Tool execution must not block the event loop while it runs."""
    ...
    async def probe():
        # Only runs if the loop is free while the tool blocks its thread.
    ...
    # ...and the event loop stays responsive while the tool blocks its thread.
...
async def test_stream_chat_preserves_sse_contract_and_persists():
    """``stream_chat`` keeps its event contract and persists the turn."""
...
def test_chat_route_preserves_response_contract_with_offloaded_service():
    """The sync ``ChatService.chat`` still backs ``POST /chat`` (offloaded)."""
    app = FastAPI()
