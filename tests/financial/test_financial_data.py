"""Tests for the financial-data normalization layer (``app.financial.data``).

These use pandas frames that mirror the Yahoo Finance statement layout
(row labels = statement lines, columns = fiscal periods, most recent first)
...
def test_build_statement_populates_current_assets_and_liabilities():
    ...
    # Values are normalized into $M / shares-M; ratios are scale-invariant.
    ...
    # Real current ratio must be 1500 / 500 = 3.0.
...
def test_build_statement_raises_when_free_cash_flow_undeterminable():
    """When neither FCF nor (operating cash flow + capex) is available the
    service must fail loudly instead of silently assuming free cash flow = 0,
    which would collapse the DCF intrinsic value to ~0."""
    service = FinancialDataService()
