"""
Tests for the canonical FinancialAssumptions model.

Covers validation bounds, provenance tracking, request overrides, and the
guarantee that duplicate module-level constants are eliminated.
"""
...
class TestFinancialAssumptionsValidation:
    """Validation bounds must reject invalid/misconfigured assumptions."""
    ...
    def test_percentage_style_value_rejected(self) -> None:
        # The classic unit confusion: 4.25 instead of 0.0425.
        with pytest.raises(ValidationError):
    ...
    def test_equity_risk_premium_derived(self) -> None:
        a = FinancialAssumptions(**self._valid())
        assert a.equity_risk_premium == pytest.approx(
            a.market_return - a.risk_free_rate
        )


class TestDefaultsAndFactory:
    """The factory builds a validated instance tagged 'configured'."""
...
class TestRequestOverrides:
    """Request-level overrides are explicit and re-validated."""
    ...
    def test_partial_override(self) -> None:
        ...
        assert b.market_return == DEFAULT_MARKET_RETURN  # inherited
    ...
    def test_override_validated_against_base(self) -> None:
        # market_return below the (unchanged) risk-free rate is invalid.
        with pytest.raises(ValidationError):
