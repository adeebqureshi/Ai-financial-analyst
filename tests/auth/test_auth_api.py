"""
Integration tests for the authentication API (auth enabled).

These tests build an isolated FastAPI application with ``AUTH_ENABLED``
forced on and a temporary SQLite account store, exercising the full
register -> login -> authorize flow plus 401 handling.
"""
...
def auth_client(tmp_path):
    """A TestClient wired to an isolated app with authentication enabled."""
    ...
    # One override drives everything: token signing secret, expiry,
    # enabled flag and the SQLite URL used by the auth session dependency.
...
class TestAuthorization:
    ...
    def test_public_endpoints_stay_open(self, auth_client):
        # Liveness/discovery endpoints must remain reachable unauthenticated.
