...
async def run():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        ...
        sc = await ac.post("/screen", json={
            ...
            "min_piotroski": 6, "min_altman": 3, "max_beneish": -1.8, "min_upside": -60,
        })
        out.append("SCREEN %s success=%s total=%s" % (sc.status_code, sc.json().get("success"), sc.json().get("data", {}).get("total")))
        ch = await ac.post("/chat", json={"message": "Analyze Apple's financial health."})
        out.append("CHAT %s success=%s msg_len=%s" % (ch.status_code, ch.json().get("success"), len(ch.json().get("data", {}).get("message", "") or "")))
