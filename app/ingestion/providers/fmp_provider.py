...
Fallback market-data provider backed by Financial Modeling Prep (FMP).

FMP is already a project-supported source (``fmp_api_key`` in settings,
``FMP_API_BASE_URL`` in constants). It offers a free tier suitable for
development and a paid tier for production. The provider reads the key from
``Settings`` and, when absent, fails fast so the provider chain can advance.
...
class FmpProvider(MarketDataProvider):
    """Fetch normalized quotes from the Financial Modeling Prep REST API."""
